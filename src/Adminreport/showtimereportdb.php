...
if(isset($_POST['submit'])){
  ...
  // Extract the date, time and theatre from the string passed in
      $d = substr($show_time,0,10);
      $t = substr($show_time,10,8);
      $theatre = substr($show_time,18,1);

      // Prepare SQL Injection protected statement to select tickets for the selected movie by theatre on the selected date and showtime, and check if it executed correctly
      $statement = $con->prepare(
        "SELECT ticket_no FROM ticket WHERE movie_name = ? AND 
      sdate = ? AND stime = ? AND theatre_no = ?");
        $statement->bind_param("ssss", $movie_name, $d, $t, $theatre);
 ...
 if (!$exec)
  ...
  if ($count == 0){
   ...
   <h1> <?php echo "No tickets exist on ".$d." ".$t." for ".$movie_name."<br>" ?> </h1>
...
<?php
  ...
  // If any tickets found, begin steps to generate report and update database
    ...
    (NULL, ?, ?, ?, 0, ?, 'admin@email.com', ?)");
      $statement2->bind_param("sssss", $d, $t, $theatre, $movie_name, $run_date);
    ...
    // Execute previous SQL injection prevention statement again to select tickets for the selected movie by theatre on the selected date and showtime, and check if it executed correctly
    $exec = $statement->execute();
    $result = $statement->get_result();
...
<h1> <?php echo "Report successfully generated! <br><br>There were $".$sales_value." in sales for <br>".$movie_name."
 on <br>".$d." for showtime at <br>".$t." in Theatre ".$theatre."!<br>" ?> </h1>
 <input type="submit" value="Back to Report Menu">
 <input type="submit" formaction="../adminindex.php" value="Back to Admin Menu">
</form>
